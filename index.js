...
regenerateBtn.addEventListener("click", () => {
  ...
  renderOutput(formatMarkdown(lastProcessedData));
...
async function runGeneration(raw) {
  ...
  // Hide overlay, render to display div, store in hidden textarea
  renderOutput(md);
...
// ── RENDER OUTPUT ─────────────────────────────────────────────
// Renders markdown into the display div with visual paragraph gaps
// and syntax highlighting, while keeping hidden textarea in sync.
function renderOutput(md) {
  // Sync hidden textarea for copy/download
  outputText.value = md;
  ...
  if (!fmMatch) {
    // No frontmatter — just show as plain paragraphs
    outputDisplay.innerHTML = "";
    buildParaBlocks(md, outputDisplay);
    return;
  }
  ...
  // ── Render frontmatter ──────────────────────────────────────
  const fmBlock = document.createElement("span");
  fmBlock.className = "block-frontmatter";

  // Opening ---
  const dashLine = document.createElement("span");
  dashLine.className = "fm-dash";
  dashLine.textContent = "---\n";
  fmBlock.appendChild(dashLine);
  ...
  fmRaw.split("\n").forEach(line => {
    ...
    if (/^\s+-\s/.test(line)) {
      sp.innerHTML =
        ...
        '<span class="fm-tag">' + esc(line.replace(/^\s+-\s/,"")) + '</span>';
    } else {
      ...
      if (colon > -1) {
        ...
        sp.innerHTML =
          ...
          '<span class="fm-sep">:</span>' +
          '<span class="fm-val">' + esc(val) + '</span>';
      } else {
        sp.textContent = line;
      }
    }
    sp.appendChild(document.createTextNode("\n"));
    fmBlock.appendChild(sp);
  });

  // Closing ---
  const dashLine2 = document.createElement("span");
  dashLine2.className = "fm-dash";
  dashLine2.textContent = "---";
  fmBlock.appendChild(dashLine2);

  outputDisplay.appendChild(fmBlock);

  // Spacer
  outputDisplay.appendChild(document.createTextNode("\n\n"));

  // ── Render body ─────────────────────────────────────────────
  buildParaBlocks(bodyRaw, outputDisplay);
}

function buildParaBlocks(text, container) {
